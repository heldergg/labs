...
class rect(BaseShape, PointAttrib, DimensionAttrib):
    ...
    def getInnerEdgePoints(self):
        ...
        result.append((float(self.get_x()) + float(self.get_width()) -
                       float(self.get_rx()), float(self.get_y()) + float(self.get_ry())))
        result.append((float(self.get_x()) + float(self.get_width()) - float(self.get_rx()),
...
class path(BaseShape, ExternalAttrib, MarkerAttrib):
    ...
    def appendCubicCurveToPath(self, controlstartx, controlstarty, controlendx, controlendy, endx, endy, relative=True):
        self.__append__('c', [controlstartx, controlstarty,
                              controlendx, controlendy, endx, endy], relative)
